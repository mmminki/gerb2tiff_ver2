...
// Aperture::modifier 리스트에서 주어진 인덱스의 값을 안전하게 반환. 범위를 벗어난 경우 예외를 발생시킴.
...
// Aperture objects.
// Aperture 객체의 링크 리스트에 새 요소를 추가하는 함수
void Aperture::render(const double dots_per_unit, const double grow_size, int ADmodifierCount)
{
	double rotation = 0; // 회전 각도 (라디안 단위)
	double standardHoleX = 0; // 표준 구멍 X 크기
	double standardHoleY = 0; // 표준 구멍 Y 크기
