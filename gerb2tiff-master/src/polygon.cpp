 ...
 */
// 다각형의 픽셀 경계값(pixelMinX, pixelMaxX 등)을 계산
// 다각형의 꼭짓점 데이터를 기반으로 스캔라인 데이터를 초기화
// 각 변(Edge)을 정의하여 스캔라인 알고리즘에 필요한 정보를 준비
 ...
 *   - Creates scan line intercept X data used for filling the polygon by scan line method.
 */
// 꼭짓점 데이터를 초기화하고 다각형 경계값 및 스캔라인 정보를 계산
// 꼭짓점 리스트를 순회하여 최소/최대 x, y값 계산
// 다각형의 모든 변을 스캔라인 알고리즘에 적합한 형태로 정렬
 ...
 */
// 주어진 각도와 반지름을 기반으로 원호 꼭짓점 추가
// 최대 허용 편차를 기반으로 꼭짓점의 수를 계산
// 원호의 모든 꼭짓점을 정밀하게 추가
